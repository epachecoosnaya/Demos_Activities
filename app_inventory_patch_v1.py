...
def almacenes():
    ...
    lista = query("""SELECT a.*,u.nombre AS resp_nombre
                     ...
                     WHERE a.activo=true ORDER BY a.codigo ASC, a.nombre ASC""", fetchall=True) or []
...
def crear_almacen():
    if not logged_in():
        return redirect(url_for("login"))
    if not tiene_permiso("crear","almacenes"):
        abort(403)
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    existe_codigo = query("SELECT id FROM almacenes WHERE codigo=%s",(codigo,),fetchone=True)
    if existe_codigo:
        flash(f"Ya existe un almacén con el código {codigo}.","danger")
        return redirect(url_for("almacenes"))
    try:
        ...
        flash(f"Almacén {codigo} creado correctamente.","success")
    except Exception as e:
        flash(f"No se pudo crear el almacén: {e}","danger")
    return redirect(url_for("almacenes"))
...
def editar_almacen(almacen_id):
    if not logged_in():
        return redirect(url_for("login"))
    if not tiene_permiso("editar","almacenes"):
        abort(403)
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
...
def crear_articulo():
    if not logged_in():
        return redirect(url_for("login"))
    if not tiene_permiso("crear","articulos"):
        abort(403)
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    codigo = request.form.get("codigo","").strip().upper()
    nombre = request.form.get("nombre","").strip()
    ...
    existe_codigo = query("SELECT id FROM articulos WHERE UPPER(codigo)=UPPER(%s)",(codigo,),fetchone=True)
    if existe_codigo:
        flash(f"Ya existe un artículo con el código {codigo}.","danger")
        return redirect(url_for("articulos"))
    existe_nombre = query("SELECT id FROM articulos WHERE UPPER(nombre)=UPPER(%s)",(nombre,),fetchone=True)
    if existe_nombre:
        flash(f"Ya existe un artículo con el nombre '{nombre}'.","danger")
        return redirect(url_for("articulos"))
    try:
        query("""INSERT INTO articulos (codigo,nombre,descripcion,grupo,categoria,uom,
              ...
              (codigo,nombre,
               ...
               now,now), commit=True)
        flash(f"Artículo {codigo} creado correctamente.","success")
    except Exception as e:
        flash(f"No se pudo crear el artículo: {e}","danger")
    return redirect(url_for("articulos"))

@app.route("/articulos/editar/<int:articulo_id>", methods=["POST"])
def editar_articulo(articulo_id):
    if not logged_in():
        return redirect(url_for("login"))
    if not tiene_permiso("editar","articulos"):
        abort(403)
    art = query("SELECT * FROM articulos WHERE id=%s",(articulo_id,),fetchone=True)
    if not art:
        abort(404)
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    codigo = request.form.get("codigo","").strip().upper()
    ...
    if not codigo or not nombre:
        flash("Código y nombre son obligatorios.","danger")
        return redirect(url_for("articulos"))
    existe_codigo = query("SELECT id FROM articulos WHERE UPPER(codigo)=UPPER(%s) AND id<>%s",(codigo,articulo_id),fetchone=True)
    if existe_codigo:
        flash(f"Ya existe otro artículo con el código {codigo}.","danger")
        return redirect(url_for("articulos"))
    existe_nombre = query("SELECT id FROM articulos WHERE UPPER(nombre)=UPPER(%s) AND id<>%s",(nombre,articulo_id),fetchone=True)
    if existe_nombre:
        flash(f"Ya existe otro artículo con el nombre '{nombre}'.","danger")
        return redirect(url_for("articulos"))
    try:
        query("""UPDATE articulos SET codigo=%s,nombre=%s,descripcion=%s,grupo=%s,categoria=%s,uom=%s,
                 precio_compra=%s,precio_venta=%s,impuesto=%s,manage_serial=%s,manage_batch=%s,
                 item_code_sap=%s,fecha_actualizacion=%s WHERE id=%s""",
              (codigo,nombre,
               request.form.get("descripcion","").strip(),
               request.form.get("grupo","").strip(),
               request.form.get("categoria","").strip(),
               ...
               request.form.get("manage_batch","N"),
               request.form.get("item_code_sap","").strip(),
               now,articulo_id), commit=True)
        flash(f"Artículo {codigo} actualizado correctamente.","success")
    except Exception as e:
        flash(f"No se pudo actualizar el artículo: {e}","danger")
    return redirect(url_for("articulos"))

@app.route("/articulos/toggle/<int:articulo_id>", methods=["POST"])
def toggle_articulo(articulo_id):
    if not logged_in():
        return redirect(url_for("login"))
    if not tiene_permiso("editar","articulos"):
        abort(403)
    art = query("SELECT id,codigo,activo FROM articulos WHERE id=%s",(articulo_id,),fetchone=True)
    if not art:
        abort(404)
    nuevo_estado = not bool(art["activo"])
    query("UPDATE articulos SET activo=%s, fecha_actualizacion=%s WHERE id=%s",
          (nuevo_estado, datetime.now().strftime("%Y-%m-%d %H:%M"), articulo_id), commit=True)
    if nuevo_estado:
        flash(f"Artículo {art['codigo']} activado correctamente.","success")
    else:
        flash(f"Artículo {art['codigo']} desactivado correctamente.","warning")
    return redirect(url_for("articulos"))
...
def inventario():
    if not logged_in():
        return redirect(url_for("login"))
    flash("Módulo de inventario en construcción. Primero cerraremos entradas y salidas.","info")
    return render_template("inventario.html", empresa=EMPRESA, logo=LOGO,
                           stock=[], sap_stock=[], tomas=[], almacenes=[],
                           almacen_id="", q="", inventario_pronto=True)
